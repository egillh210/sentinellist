...
contract SentinelList4337Test is Test {
    ...
    function test_Pop_PoppingAllEntriesShouldSetSentinelToInitState() external {
        list.push(account, one);
        list.push(account, two);
        list.push(account, three);

        list.pop(account, SENTINEL, three);
        list.pop(account, SENTINEL, two);
        list.pop(account, SENTINEL, one);
